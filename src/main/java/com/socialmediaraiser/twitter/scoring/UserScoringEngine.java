...
public class UserScoringEngine {
    ...
    public int getUserScore(AbstractUser user){
        FollowProperties.getScoringProperties().getProperty(Criterion.NB_FOLLOWERS).setValue(user.getFollowersCount());
    ...
    private int getNbFollowersScore(int nbFollowers){
        int maxPoints = FollowProperties.getScoringProperties().getProperty(Criterion.NB_FOLLOWERS).getMaxPoints();
